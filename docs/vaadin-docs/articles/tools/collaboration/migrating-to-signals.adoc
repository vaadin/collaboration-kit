...
Collaboration Kit assigns each user a color index on first sight, handing out the seven available values in order, so users active at the same time reliably get distinct colors. The registry that does this lives in [classname]`CollaborationEngine`.

Nothing equivalent ships with signals. Hashing the user identifier, as <<#step-2-replace-userinfo,Step 2>> does, is stable and needs no coordination, but two users in the same topic can collide. Allocating indexes from the collaborator list when a user joins restores the guarantee at the cost of writing the allocator.
...
Collaboration Kit accepts a Jackson [classname]`TypeReference` wherever it accepts a [classname]`Class`, which is how [methodname]`CollaborationMap::get` reads a `Set<String>` and how [methodname]`CollaborationBinder::forField` binds a [classname]`CheckboxGroup` by taking the collection type and the element type separately.

Shared signals take a [classname]`Class` only. There's no [classname]`TypeReference` overload anywhere in the API, so a parameterized value type can't be expressed: `new SharedValueSignal<>(Set.class)` loses the element type, and reading it back loses the element type.
...
==== No Previous Value or Change Type

A Collaboration Kit subscriber receives an event, and the event describes the change: [classname]`MapChangeEvent` carries the old value next to the new one, and [classname]`ListChangeEvent` adds the change type along with the previous and next keys before and after the change. Code that animates a delta, logs an edit history, or reacts only to a specific kind of change reads those fields.
...
Keep the previous value yourself, in a second signal updated from the effect. The <<{articles}/flow/ui-state/usage-examples/realtime-dashboard#,real-time dashboard example>> shows the pattern: a `Change` record holding the previous and current values, written with [methodname]`peek()` so the effect doesn't depend on its own output. Reconstructing a list change type from before-and-after snapshots is more work, and if the code genuinely needs it, the structure is a poor fit for signals -- consider an append-only [classname]`SharedListSignal` of change records instead.
...
| Previous value and change type
| Missing
